...
const LoginScreen = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const { id, token, firstName, profilePicture, trustScore } = data;
        await AsyncStorage.setItem('trustScore', trustScore.toString());
        console.log(data)

        // Check if the values are not undefined before saving
        if (id && token && firstName && profilePicture) {
          await AsyncStorage.setItem('token', token);
          await AsyncStorage.setItem('firstName', firstName);
          await AsyncStorage.setItem('profilePicture', profilePicture);
          login(id, token, firstName, profilePicture, trustScore, 3600); // Set user information in context
        } else {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        navigation.navigate('ConfirmResetCode'); // <-- create this screen next
      } else {
        Alert.alert('Error', message || 'Failed to send SMS.');
      }
    } catch (error) {
      Alert.alert('Error', 'Could not send password reset SMS.');
    }
  };
  ...
  <Text style={styles.linkText}>Forgot Password?</Text>
      ...
      <View style={styles.footer}>
        <TouchableOpacity onPress={() => Linking.openURL('https://www.billixapp.com/')}>
